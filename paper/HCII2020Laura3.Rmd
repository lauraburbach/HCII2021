...
## Netlogo
Different platforms to create agent-based models exist. NetLogo consists of a programming language, which was derived from the earlier Logo language and a set of libraries, as well as a programming environment.

however NetLogo also provides a graphical tool for quickly constructing interfaces for running agent-based models.
...
To built the agent based models, we used either the Julia programming language or Netlogo. For Julia we used the Atom editor. We used version 6.0.4 of the multi-agent programming language Netlogo, which was developed by Uri Wilensky [@Wilensky1999]. For the following analysis of the results we used R Markdown. 

## Basics of Agent-based models 
...
## Netlogo
Netlogo as a modelling language for agent-based modeling is very well suited for beginners wanting to use agent-based modeling. It comes with a rich variety of example models that users can explore and provides a graphical user interface and a graphical user interface toolkit to create models that even non-experts can use. 
Netlogo provides methods for inspecting the model (reporters and visualizations) and for exploring the impact of model parameters on system behavior (i.e. the *behavior space* feature). The latter allows turning of the GUI for faster simulations (see Fig. \@ref(fig:nbehspc)).
...
This means the user has to design all tools for agent-based modeling themselves. However, this is not necessarily very hard. It depends on the complexity of the model. When this barrier has been overcome, writing a model becomes easier. The biggest challenge was to find a structure that for exploring parameter spaces. In our approach, we created a set of nested functions that pass the full state of the simulation. Then from the outside we used list comprehension to generate parameter sets. 

Julia provides methods for paralellization using simple macros (code annotations.) This makes preparing code for large scale simulations very easy develop. While the entry barrier significantly is higher for Julia, due to a complex type system, its performance and general purpose approach make it easy to integrate other peoples work. A key benefit are the Python and R integrations that allow using Python and R libraries in Julia.

```{r, out.width="1\\linewidth", include=TRUE, fig.align="center", fig.cap=c("main procedure in julia"), echo=FALSE}
knitr::include_graphics("../main_julia.png")
```

In the following section, we will compare different procedures between both versions of our model.


## Go procedure 
Here we compare the "go" procedures, so what happens in each step of the simulation, of Netlogo and Julia (see Figure 3 and 4). Both codes look similar. In Netlogo (see Figure 4), the procedure starts by adressing the agents (ask turtles). The next line of code says, that the adressed agent gets the opinion of one random other agent. The subsequent lines of code determine what happens to the (new) opinion of the agent. If the other agent's opinion differs less from his own opinion than the epsilon (see above), the agent assumes the average opinion of the two opinions. This means that the opinions of the two agents are added together and divided by two. However, if the opinion of the other agent is further away than the respective (may vary) epsilon indicates, it checks whether the backfire effect exists. If the simulation is set to show that the effect exists, the opinion of the agent is half the distance away from the opinion of the other agent. At the end, the code indicates that the color of the agents depends on the opinion. However, this is only for illustration in the interface. Before the procedure ends, one more "tick" is counted as one time unit. 
```{r, out.width="1\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Go procedure in julia"), echo=FALSE}
...
```{r, out.width="1\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Go procedure in Netlogo"), echo=FALSE}
...
The results of our research have shown that, although Netlogo is established for a longer time, both programming languages are well-suited to create agent-based models. Comparing the two languages, we could not find one perfect language, but each laguage is the better choice for creating an agent-based model in some aspects. The decision for a programming language depends on different trade-offs (previous experience vs. support; time to create the model vs. time used for simulation run; nice Interface vs. higher functionality). In the end, however, it does not make sense to decide in favor of one language against the other, but to take advantage of both languages and thus use Netlogo for prototyping and Julia for large simulations based on it. 
