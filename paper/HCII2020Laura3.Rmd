...
The most established language or program for creating agent-based models is Netlogo. ...

But while Netlogo is used since..., the spread of the Internet also resulted in the evolving of different programming languages [@Valdez2018human]. 

Besides Netlogo, there are also other languages that can be used to create agent-based models. 

So far, it has not been considered which language is actually best suited for creating agent-based models. Therefore, in this study we investigate whether Netlogo or Julia is better suited for creating agent-based models. 
...
## Complexity and how to modell it
...
This last case can be described by a so-called bounded confidence model.
Models like this haven been proposed by both @Hegselmann2002 and  @Nadal2002.
Here, the agent will only interact with agents whose opinion is relatively close to their own.
...
<!--- Add something on the size of confidence level apart from interesting math simulations, add something on the backfire effect --->
...
```{r, out.width="1\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Behavior Space in Netlogo"), echo=FALSE}
knitr::include_graphics("../BehaviorSpace2.png")
```


## Go procedure 
Here we compare the "go" procedures of Netlogo and Julia. Both codes look similar. In Netlogo (see Figure X), the procedure starts by adressing the agents (ask turtles)
